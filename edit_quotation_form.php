...
if (isset($_POST["submit"])) {
    ...
    $invoice_no = mysqli_real_escape_string($conn, $_POST["quotation_no"]);
    $invoice_date = date("Y-m-d", strtotime($_POST["quotation_date"]));
    ...
    $sql = "UPDATE quotation SET 
        ...
        if ($conn->query($sql)) {
            ...
            if (!empty($updateRows)) {
                $updateSql = "INSERT INTO quservice (Id, Sid, description, HSN, UOM, Qty, price, amount, rate_of_tax, taxable_value, total_value_amount) VALUES ";
            ...
            // Insert new rows
            if (!empty($insertRows)) {
                $insertSql = "INSERT INTO quservice (Sid, description, HSN, UOM, Qty, price, amount, rate_of_tax, taxable_value, total_value_amount) VALUES ";
